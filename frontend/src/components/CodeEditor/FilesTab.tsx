...
/**
 * The FILES tab: a file-tree rail and a tabbed viewer area. Owns the open-tabs /
 * active-tab / dirty state, hydrates it once from the persisted editor state, and
 * writes it back when the user changes tabs. Picks the viewer per file kind
 * (§14.1, §15.1). Extracted from the old CodeEditorView body so the takeover can
 * host sibling tabs (REPO/PRS/TERMINAL); the back button + project name now live
 * in the shell header.
 */
interface Props {
  projectId: number;
